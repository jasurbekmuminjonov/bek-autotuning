...
const Inprogress = () => {
    ...
    useEffect(() => {
        const filtered = projects.filter(project =>
            project.services_providing.some(service => service.user_id === userId && service.status === "inprogress" || service.status === "rejected")
        );
                    ...
                    userProjects?.map((item) => (
                        <div key={item._id} className="user_project">
                            ...
                            <div className="project_services">
                                <p>Faol servislar</p>
                                {
                                    item.services_providing.filter(service => service.user_id === userId && service.status === "inprogress" || service.status === "rejected").map((service) => (
                                        <div key={service._id} className="service_card">
                                            <p>Servis: {services.find(s => s._id === service.service_id).service_name}</p>
                                            <p>Holati: {service.status === "inprogress" ? "Jarayonda" : service.status === "rejected" ? "Rad etilgan" : "-"}</p>
                                            <p>Boshlangan sana: {moment(service.started_time).format("DD.MM.YYYY HH:mm")}</p>
                                            {service.status === "rejected" && <p>Tugatilgan sana: {moment(service.ended_time).format("DD.MM.YYYY HH:mm")}</p>}
